...
function Subscribers() {
  const subscriberHeader = ["ID", "Name", "Email", "Phone", "Date", ""];
  const properties = ["id", "name", "email", "phone", "date"];
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 7;
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = SubscriberRowData.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
        ...
        <div className="border-t-0 border-x-2 border-b-2 border-[#311A67]">
          ...
          <div className="px-10 py-4 flex justify-between">
            <p className="text-textWhite font-semibold font-[Barlow]">
              Showing {itemsPerPage} out of {SubscriberRowData.length}
            </p>
            <Pagination
              itemsPerPage={itemsPerPage}
              totalItems={SubscriberRowData.length}
